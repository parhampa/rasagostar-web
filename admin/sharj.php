...
<div class="w3-main" style="margin-right:300px;margin-top:43px;">
    ...
    <div class="w3-white w3-padding-large w3-margin w3-round-medium w3-right" style="width: 80%;">
        ...
        $fm->label("شماره موبایل", "w3-text-green")
            ...
            ->inptype("text")
            ->end()
            ->sndform("mobile", 0, 1, "شماره موبایل", 1, 1);
        $fm->label("مبلغ", "w3-text-green")
            ->input()
            ->inpname("price")
            ->inpid("price")
            ->inpclasses("w3-input w3-border")
            ->inptype("number")
            ->end()
            ->sndform("price", 0, 1, "مبلغ", 1);
        $fm->label("وضعیت شارژ", "w3-text-green")
            ->select()
            ->selectname("vaz")
            ->selectid("vaz")
            ->selectclasses("w3-select w3-border")
            ->selectaddval("0", "در حال بررسی")
            ->selectaddval("1", "تایید شده")
            ->selectaddval("2", "رد شده")
            ->end()
            ->sndform("vaz", 2, 1, "وضعیت شارژ", 1, 1);
        $fm->label("توضیحات", "w3-text-green")
            ->texarea()
            ->areaname("txt")
            ->areaid("txt")
